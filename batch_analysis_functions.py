...
def print_info(df,info):
    str='-----'
    for i in range(len(info)):
        str+=info[i]+': '+str(row[info[i]])+'__'
    print(str)

def search_for_options(options_list,options_names,row,f_options):
    for i in range(len(options_list)):
        if not pd.isna(row[options_list[i]]):
            f_options[options_names[i]]=row[options_list[i]]
    return f_options
